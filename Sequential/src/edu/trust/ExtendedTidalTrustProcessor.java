 ...
 * the users that have rated the item using Golbeck's Tidal Trust algorithm. It
 * returns the users that has rated the item at the minimum possible depth from
 * the user.
 * 
...
public class ExtendedTidalTrustProcessor {
	...
	public static UserTrustResponse getTrustRatings(UserTrustRequest req) {
		...
		try {
			...
			while (!toProcessQueue.isEmpty() && currentDepth <= maxDepth) {
				...
				usersAtDepthMap = addUserToDepthMap(usersAtDepthMap,
						currentDepth, processUser);
				...
				for (Long userId : trustedUsers) {
					...
					UserItemRating userItemRating = DBConnectionManager
							.getUserItemRatingByUserIdItemId(userId,
									req.getItemId());
					if (userItemRating != null
							&& userItemRating.getItemId() > 0L
							&& processUser.getUserTrustMap().get(userId) > 0F) {// Found
																				// sink
						foundSink = true;
						userDestRating = addDestRating(
								userDestRating,
								processUser,
								new DestinationRating(tUser, processUser
										.getUserTrustMap().get(
												tUser.getUserId())));
						destUsers.add(tUser.getUserId());
						maxDepth = currentDepth;
						Float currentFlow = getMinimunFlow(
								userPathFlow,
								processUser,
								processUser.getUserTrustMap().get(
										tUser.getUserId()));
				...
				if (!foundSink && currentDepth < maxDepth) {

					for (Long userId : trustedUsers) {
						UserBo tUser = userBoMap.get(userId);
						if (tUser == null) {
							continue;// This happens if the user doesn't rate
										// any items and doesn't trust any users
						}
						...
						if (nonVisitedUsersAtCurrentDepth.contains(tUser)) {
							Float currentFlow = getMinimunFlow(userPathFlow,
									processUser, processUser.getUserTrustMap()
											.get(tUser.getUserId()));
							Float tUserFlow = userPathFlow.get(tUser);
							if (tUserFlow != null) {
							...
							userPathFlow.put(tUser, tUserFlow);
							// Add child node to user children hierarchy with
							// processUser as parent
							addUserChildren(userChildren, processUser, tUser);
						}
					}
				}
			...
			for (Long destUserId : destUsers) {
				...
				if (!usersAtDepthMap.get(runningDepth).isEmpty()) {
					for (UserBo user : usersAtDepthMap.get(runningDepth)) {
						if (user.getUserId() == userBo.getUserId()
								&& user.getTrustedUsers().contains(destUserId)) {
							foundAtDepthOne = true;
							if (user.getUserTrustMap().get(destUserId) >= max) {
								trustMap.put(destUserId, user.getUserTrustMap()
										.get(destUserId));
								break;
							}
						}
				...
				if (!foundAtDepthOne) {
					while (runningDepth > 0) {
						if (!usersAtDepthMap.get(runningDepth).isEmpty()) {
							for (UserBo user : usersAtDepthMap
									.get(runningDepth)) {
								Float numer = 0.0F;
								Float den = 0.0F;
								if (null != userChildren) {
									for (UserBo childUser : userChildren
											.get(user)) {
										if (user.getUserTrustMap().get(
												childUser.getUserId()) >= max
												&& userDestRating
														.get(childUser) != null) {
											for (DestinationRating destRating : userDestRating
													.get(childUser)) {
												if (destRating.destination
														.getUserId() == destUserId
	...
	private static Map<UserBo, List<UserBo>> addUserChildren(
			Map<UserBo, List<UserBo>> userChildren, UserBo parent, UserBo child) {
	...
	private static Map<Integer, List<UserBo>> addUserToDepthMap(
			Map<Integer, List<UserBo>> usersAtDepthMap, int depth, UserBo user) {
	...
	private static Float getMinimunFlow(Map<UserBo, Float> userPathFlow,
			UserBo user, Float rating) {
	...
	private static Map<UserBo, List<DestinationRating>> addDestRating(
			Map<UserBo, List<DestinationRating>> userDestRating, UserBo user,
			DestinationRating dest) {
